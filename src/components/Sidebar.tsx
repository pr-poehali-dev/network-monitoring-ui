    ...
    <div className="fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-gray-200">
      ...
      <nav className="mt-6 px-3">
        <div className="space-y-1">
          ...
          {/* Текущая станция */}
          {isStationPage && (() => {
            const serialNumber = location.pathname.split('/station/')[1];
            const currentStation = stations.find(s => s.station_id === serialNumber);
              ...
              <div className="mt-4 pt-4 border-t border-gray-200">
                ...
                <div className="flex items-center gap-3 px-3 py-2.5 text-sm font-medium bg-green-50 text-green-700 rounded-lg border-r-2 border-green-600">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-green-600">{currentStation.ip_address || 'IP не указан'}</div>
                  </div>
                </div>
              </div>
            );
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between text-sm">
              ...
              <span className="font-medium text-green-600">
                {stations.filter(s => s.station_status === 'connected').length}
              </span>
            </div>
            <div className="flex items-center justify-between text-sm">
              <span className="text-gray-600">В ошибке</span>
              <span className="font-medium text-orange-600">
                {stations.filter(s => s.station_status === 'error').length}
              </span>
            </div>
            <div className="flex items-center justify-between text-sm">
              <span className="text-gray-600">Оффлайн</span>
              <span className="font-medium text-red-600">
                {stations.filter(s => s.station_status === 'disconnected').length}
              </span>
            </div>
            <div className="flex items-center justify-between text-sm">
              <span className="text-gray-600">Заряжают</span>
              <span className="font-medium text-blue-600">
                {stations.filter(s => 
                  s.connectors?.some(c => c.status === 3)
                ).length}
              </span>
            </div>
          </div>
